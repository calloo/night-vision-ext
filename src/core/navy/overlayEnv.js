...
import { fastSma, candleColor, rescaleFont } from '../primitives/navyLib/helperFns.js'
...
export default class OverlayEnv {
  ...
  constructor(id, ovSrc, layout, props) {
    ...
    this.$core = {
      ...
      scan,
      events,
      repaint: () => hub.se.chart.update(),
      disableChartPanning: () => {
        hub.se.chart.enableGridPanning = false
      },
      enableChartPanning: () => {
        hub.se.chart.enableGridPanning = true
      },
    }
  ...
  update(overlay, layout, props) {
    ...
    let core = this.$core
    core.layout = this.buildLayout(layout, props.range, overlay)
  ...
  buildLayout(layout, range, overlay) {
    ...
    return layoutFn(Object.assign(obj, layout, s), range, overlay)
  }
